...
def w2v_weight_build(words):
    ...
    w2vModel = KeyedVectors.load_word2vec_format('word2vec/cnews-word2vec-cbow/word2vec.txt')
    index = -1
    cnt_right = 0
    cnt_wrong = 0
    for word in words:
...
def train():
    ...
    print(base_dir)
    print('#####################################################')
    x_train_c, y_train = data_loader.process_file(train_dir, character_to_id, cat_to_id, config.seq_length_c)
    x_train_w, _ = data_loader_wordlevel.process_file(train_dir, word_to_id, cat_to_id, config.seq_length_w)

    x_val_c, y_val = data_loader.process_file(val_dir, character_to_id, cat_to_id, config.seq_length_c)
    x_val_w, _ = data_loader_wordlevel.process_file(val_dir, word_to_id, cat_to_id, config.seq_length_w)
...
def test():
    ...
    start_time = time.time()
    x_test_c, y_test = data_loader.process_file(test_dir, character_to_id, cat_to_id, config.seq_length_c)
    x_test_w, _ = data_loader_wordlevel.process_file(test_dir, word_to_id, cat_to_id, config.seq_length_w)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(vocab_dir_w):  # 如果不存在词汇表，重建
        data_loader_wordlevel.build_vocab(train_dir, vocab_dir_w, config.vocab_size_w)
